...
public class PersonSummary extends PersonRelationalComponent implements HyperlinkListener{
	...
	result += "<span class=\"" + Person.PROPERTY_GENDER + "\">" + person.getGender().getSymbol() //$NON-NLS-1$ //$NON-NLS-2$
		...
		? "○ " + person.getDateOfBirth().toString() + (person.getDateOfDeath() != null ? " " : "") : "") //$NON-NLS-2$//$NON-NLS-3$ //$NON-NLS-4$
		+ "</span>"
		+ "<span class=\"" + Person.PROPERTY_DATEOFDEATH + "\">" //$NON-NLS-1$//$NON-NLS-2$
		+ (person.getDateOfDeath() != null ? "† " + person.getDateOfDeath().toString() : "") //$NON-NLS-1$//$NON-NLS-2$
		+ "</span>" //$NON-NLS-1$
		+ (person.getDateOfBirth() != null || person.getDateOfDeath() != null ? ")" : "");
